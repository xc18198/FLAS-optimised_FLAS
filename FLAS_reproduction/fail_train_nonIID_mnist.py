...
def check_data_loss(model,dl_train):
    ...
    for x,y in dl_train:
        ...
        acc = (pred.argmax(dim=1)==y).cpu().float().numpy()
...
model0 = client_model_lenet5()
for i in range(n_models):
    model = client_model_lenet5()
...
datalist = genSelectList(len(data),n_models,0.2,IID=False,data=data)

traindata = {}
testdata = {}
for i in range(len(datalist)):
    traindata[models[i][0]] = MNISTDataset('../data',datalist[i][0])
    testdata[models[i][0]] = MNISTDataset('../data',datalist[i][1])
...
# his = train(model,train_loader[0],test_loader[0],epochs = 20,start=1)
...
for g_epoch in range(g_max_epoch):
    ...
    for model_no,(model_name,model) in enumerate(models):
        ...
        # pdb.set_trace()
        dl_train = DataLoader(traindata[model_name],batch_size = 10,shuffle = False,num_workers=0)
        global_loss[model_name],global_acc[model_name] = check_data_loss(local_models[model_name],dl_train)
        model_server.upload_global_loss(model_name,global_loss[model_name].mean(),global_acc[model_name].mean())
    ...
    for model_no,(model_name,model) in enumerate(models):
        ...
        loss = train(model_name,model,traindata[model_name],testdata[model_name],g_epoch,local_epochs = 5,lr=1e-4)
...
pickle.dump(model_server,open('model_server_nonIID_mnist.pkl','wb'))


pl.plot(model_server.global_acc)
pl.xlabel('Training epoch')
pl.ylabel('Accuracy')
pl.savefig('nonIID_mnist.jpg')
pl.show()
